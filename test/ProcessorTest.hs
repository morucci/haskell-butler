    ...
    testGroup
            ...
            withProcessor' $ \_ _ _ startProcess' -> do
                ...
                case status of
                    Stopped (end, reason) -> do
                        reason @?= Exited
                        assertBool ("Invalid process end time") (from @Time @Milli end < 3 && from @Time @Milli end > 0)
        ...
        , testCase "Create tasks" do
            withProcessor' $ \_ logger processor startProcess' -> do
                ...
                WaitCompleted _ <- join (atomically <$> waitTransaction 100 (awaitProcessor processor))
                pure ()
        ]
  where
