...
newGuiApp name size draw triggerNames cb = newGuiApp2 name size emptyDraw emptyDraw draw triggerNames cb
...
hxClick :: Monad m => Text -> _ -> _ -> HtmlT m () -> HtmlT m ()
hxClick name cls elem body = with elem [wsSend, hxTrigger_ "click", id_ name] body
