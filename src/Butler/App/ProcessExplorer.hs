...
renderPE :: Desktop -> TVar PEState -> WinID -> ProcessIO (HtmlT STM ())
renderPE desktop stV wid = do
    os <- asks os
    st <- readTVarIO stV
    ...
    let renderProcessM pid = do
            processM <- lift (getProcess os.motherboard.processor pid)
            li_ do
                case processM of
                    Nothing -> "ghost"
                    Just process -> renderProcess process
...
peApp desktop wid = do
    ...
    newGuiApp "ps" Nothing (const $ renderPE desktop stV wid) (scopeTriggers wid ["ps-kill", "ps-toggle"]) \app -> do
                ...
                Nothing -> case withoutWID ev.trigger of
                    "ps-toggle" -> do
                      ...
                      atomically $ writeTVar stV $ case st of
                        ...
                        PEScopped _ -> PEAll
                      broadcastMessageT desktop =<< renderPE desktop stV wid
                    _ -> logError "unknown event" ["ev" .= ev]
        forever do
            os <- asks os
            sysEvent <- join (atomically <$> waitLog os.motherboard.logger 10_000 isProcess)
            case sysEvent of
                WaitCompleted{} -> do
                    broadcastMessageT desktop =<< renderPE desktop stV wid
                WaitTimeout{} -> pure ()
