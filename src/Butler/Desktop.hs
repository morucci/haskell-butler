...
deleteApp desktop pid = do
    ...
    atomically do
        ...
        void $ stopProcess os.motherboard.processor pid
...
desktopProgram appLauncher xinit = startDisplay 8080 \display -> do
        ...
        getDesktop name = modifyMVar workspaces $ \wss -> case Map.lookup name wss of
            ...
            Nothing -> do
                ...
                void $ spawnProcess (ProgramName $ "desktop-" <> from name) do
                    processEnv <- ask
                    baseStorage <- mkDir processEnv.os.motherboard.storage $ StorageAddress (via @Text name)
    ...
    rootProcess <- spawnProcess "welcome-desktop" do
...
handleWelcomeEvents workspaces client trigger ev = case ev ^? key "ws" . _String of
    Just wsTxt -> do
        ...
        case trigger of
            ...
            "new-ws" -> do
                let cleanWS = Text.takeWhile isAlphaNum wsTxt
                case cleanWS of
